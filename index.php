...
$loginMessage = '';
if (isset($_GET['msg']) && $_GET['msg']==='logout') $loginMessage='Sessão encerrada.';
if ($_SERVER['REQUEST_METHOD']==='POST') {
    if (!csrf_validar($_POST['csrf']??null)) { $loginMessage='Token inválido.'; }
    else {
        $email = filter_var($_POST['email']??'', FILTER_SANITIZE_EMAIL);
        $senha = $_POST['senha']??'';
        if ($controller->login($email,$senha)) {
            if (($_SESSION['usuario_tipo']??'')==='admin') header('Location: View/admin/painel_admin.php');
            else header('Location: View/painel_aluno.php');
            exit;
        }
...
<figure class="bgLinearGradient rounded-circle d-flex justify-content-center align-items-center"><i class="bi bi-lightning-charge-fill fs-4 text-white"></i></figure>
...
<input type="password" name="senha" class="form-control border-start-0 p-2" placeholder="Sua senha" required>
</div>
</div>
...
<script>
<?php if($loginMessage): ?>
Toastify({text:<?= json_encode($loginMessage) ?>,duration:3000,close:true,gravity:"bottom",position:"right",style:{background:"hsl(6 78% 57%)"}}).showToast();
<?php endif; ?>
</script>
</body>
</html>
