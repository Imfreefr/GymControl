...
use Controller\AlunoController; use Model\Evolucao;
exigirLogin();
if(($_SESSION['usuario_tipo']??'')==='admin'){ header('Location: admin/painel_admin.php'); exit; }
$alunoCtrl=new AlunoController(); $evo=new Evolucao();
...
if($_SERVER['REQUEST_METHOD']==='POST'){
    if(!csrf_validar($_POST['csrf']??null)) $msg='Token inválido.';
        ...
        if($ok) header('Location: evolucao.php'); 
    }
}
...
<form method="POST">
<div class="mb-2"><label class="form-label small">Peso (kg)</label><input type="number" step="0.1" name="peso" class="form-control" placeholder="78.5"></div>
<div class="mb-2"><label class="form-label small">Altura (m)</label><input type="number" step="0.01" name="altura" class="form-control" placeholder="1.75"></div>
<div class="mb-2"><label class="form-label small">Data</label><input type="date" name="data" class="form-control" value="<?= date('Y-m-d') ?>"></div>
<div class="mb-2"><label class="form-label small">Observação</label><input type="text" name="observacao" class="form-control" placeholder="Ex: Após 30 dias"></div>
...
<div class="card card-gym border-0 mb-3"><div class="card-body"><canvas id="grafPeso" height="140"></canvas></div></div>
<?php endif; ?>
<div class="card card-gym border-0">
<div class="card-body">
...
<thead><tr><th>Data</th><th>Peso</th><th>Altura</th><th>Obs</th></tr></thead>
<tbody>
<?php if(empty($historico)): ?><tr><td colspan="4" class="small text-muted">Nenhum registro.</td></tr>
<?php else: foreach($historico as $h): ?><tr><td><?= e($h['data']) ?></td><td><?= e($h['peso']) ?> kg</td><td><?= e($h['altura']) ?> m</td><td class="small"><?= e($h['observacao']??'—') ?></td></tr><?php endforeach; endif; ?>
</tbody>
</table>
</div>
</div>
...
new Chart(document.getElementById('grafPeso'),{type:'line',data:{labels,datasets:[{label:'Peso (kg)',data:pesos,borderColor:'#ea580c',backgroundColor:'rgba(234,88,12,.12)',tension:.3,fill:true}]},options:{plugins:{legend:{display:false}}}});
</script>
<?php endif; ?>
